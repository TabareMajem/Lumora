...
import { z } from 'zod';
import { Video, Camera, Users, PenTool, Palette, Music, Star } from 'lucide-react';
import { Card, CardHeader, CardContent } from '../ui/Card';
import { Button } from '../ui/Button';

const ratingSchema = z.object({
  direction: z.number().min(1).max(10),
  cinematography: z.number().min(1).max(10),
  acting: z.number().min(1).max(10),
  screenplay: z.number().min(1).max(10),
  artDirection: z.number().min(1).max(10),
...
interface FilmRatingFormProps {
  filmId: string;
  onSubmit: (data: RatingFormData) => Promise<void>;
  initialRatings?: Partial<RatingFormData>;
}

export function FilmRatingForm({ filmId, onSubmit, initialRatings }: FilmRatingFormProps) {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const ratingCategories = [
    ...
    { name: 'cinematography', label: 'Cinematography', icon: Camera },
    { name: 'acting', label: 'Acting', icon: Users },
    { name: 'screenplay', label: 'Screenplay', icon: PenTool },
    { name: 'artDirection', label: 'Art Direction', icon: Palette },
  ...
  return (
    <Card>
      <CardHeader>
        <h3 className="text-lg font-semibold">Rate This Film</h3>
      </CardHeader>
      <CardContent>
